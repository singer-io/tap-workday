...
from tap_workday.streams.abstracts import ParentBaseStream
...
class TestSync(unittest.TestCase):
    ...
    @patch(
        "tap_workday.streams.abstracts.ParentBaseStream.is_selected", return_value=True
    )
    @patch(
        "tap_workday.streams.abstracts.ParentBaseStream.get_bookmark", return_value=100
    )
    def test_get_bookmark_parent_only_selected(
        self, mock_get_bookmark, mock_is_selected
    ):
    ...
    @patch("tap_workday.streams.abstracts.BaseStream.is_selected", return_value=False)
    @patch(
        "tap_workday.streams.abstracts.IncrementalStream.get_bookmark", return_value=100
    )
    def test_get_bookmark_parent_only_but_not_selected(
        self, mock_get_bookmark, mock_is_selected
    ):
    ...
    @patch("tap_workday.streams.abstracts.BaseStream.is_selected", return_value=True)
    @patch(
        "tap_workday.streams.abstracts.IncrementalStream.get_bookmark",
        side_effect=[100, 50, 75],
    )
    ...
    def test_get_bookmark_only_children_selected(
        self, mock_get_bookmark, mock_is_selected
    ):
